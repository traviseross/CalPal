#!/usr/bin/env python3
"""
Database-Aware 25Live Sync Service

Syncs events from 25Live to Google Calendars with database tracking:
- Classes: Travis's teaching schedule → Classes calendar
- GFU Events: University events → GFU Events calendar

All events are recorded in PostgreSQL database for tracking and lifecycle management.
            ...
            for start_date, end_date in date_ranges:
                ...
                for reservation in reservations:
                    try:
                        ...
                        # FALLBACK: Check Google Calendar directly (for events not in DB)
                        existing_on_gcal = self.check_google_calendar_for_event(
                            calendar_id,
                        ...
                        if existing_on_gcal:
                            # Event exists on Google but not in DB - add to DB
                            self.logger.info(f"Found existing event on Google Calendar (not in DB): {event_data['summary']}")
                            event_data['event_id'] = existing_on_gcal
                            event_data['ical_uid'] = None  # Will be populated on next scan
                        ...
                        google_event_id, ical_uid = self.create_google_calendar_event(event_data)
                        ...
                        # Record in database
                        event_data['event_id'] = google_event_id
